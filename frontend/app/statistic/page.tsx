...
export default function Page() {
  ...
  return (
    <main className={`min-h-screen flex flex-col ${poppins.className}`}>
        <Navbar/>
        <div className="grid grid-cols-3 flex-grow gap-5">
          ...
          <ul className="menu rounded-box w-full shadow-xl px-20 py-30 flex flex-col justify-between">
              <li className={`mb-10 text-xl ${selectedMenu === 'Descriptive' ? 
              ...
              <li className={`mb-10 text-xl ${selectedMenu === 'Statistic 2' ? 
              'bg-[#FF6B6B] font-bold rounded-full py-2 px-4 text-white' : ''}`}
                  onClick={() => setSelectedMenu('Statistic 2')}>
                <a>
                <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5"
          ...
          <div className="col-span-2 px-10 py-10">
            ...
            {selectedMenu === 'Statistic 2' && <div>Coming Soon...</div>}
            {selectedMenu === 'Statistic 3' && <div>Coming Soon...</div>}
          </div>
        </div>
    </main>
  );
